...
export const ReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const allMeetings = meetingsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Meeting));
        ...
        const depts = deptsSnapshot.docs.map(doc => ({ id: doc.id, name: doc.data().name }));
  ...
  const prepareChartData = (tasks: Task[], meetings: Meeting[], users: User[], start: Date, end: Date) => {
    ...
    const labels = days.map(day => format(day, 'dd/MM', { locale: arSA }));
    ...
    const meetingsByDay = days.map(day => {
      const dayStr = format(day, 'yyyy-MM-dd');
      return meetings.filter(m => m.date === dayStr).length;
    });
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      switch (activeReport) {
        ...
        case 'performance':
          data = [
            { المقياس: 'إجمالي المهام', القيمة: stats.totalTasks },
            { المقياس: 'المهام المكتملة', القيمة: stats.completedTasks },
            { المقياس: 'نسبة الإنجاز', القيمة: `${stats.completionRate.toFixed(1)}%` },
            { المقياس: 'متوسط وقت الإنجاز', القيمة: `${stats.avgCompletionTime.toFixed(1)} ساعة` },
            { المقياس: 'إجمالي الاجتماعات', القيمة: stats.totalMeetings },
            { المقياس: 'المستخدمين النشطين', القيمة: stats.activeUsers },
            { المقياس: 'أفضل إدارة', القيمة: stats.topPerformingDepartment },
            { المقياس: 'أفضل موظف', القيمة: stats.topPerformingUser }
          ];
          break;
        case 'departments':
  ...
  const StatCard: React.FC<{ title: string; value: string | number; icon: React.ReactNode; color: string; trend?: number }> = ({ title, value, icon, color, trend }) => (
  ...
  const ChartView: React.FC = () => {
    const maxValue = Math.max(...chartData.tasks, ...chartData.meetings, ...chartData.users, 1);
          ...
          <tbody>
            {usersData.filter(u => selectedDepartment === 'all' || u.department === selectedDepartment).map(user => (
              <tr key={user.uid} className="border-b hover:bg-hv transition-colors" style={{ borderColor: 'var(--bd)' }}>
                ...
                <td className="p-3">
                  <span className={`badge ${user.primaryRole === 'chairman' ? 'badge-primary' : user.primaryRole === 'manager' ? 'badge-info' : 'badge'}`}>
                    {user.primaryRole === 'chairman' ? 'رئيس' : user.primaryRole === 'vp' ? 'نائب رئيس' : user.primaryRole === 'manager' ? 'مدير' : 'موظف'}
                  </span>
                </td>
                <td className="p-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={exportToCSV} disabled={exporting} className="btn-secondary">
            {exporting ? <FaSpinner className="animate-spin" /> : <FaDownload className="ml-2" />}
            تصدير
          </button>
          ...
          <div className="flex flex-wrap gap-2">
            {(['today', 'week', 'month', 'quarter', 'year', 'custom'] as DateRange[]).map(range => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-sm transition-all ${dateRange === range ? 'bg-brand text-white' : 'hover:bg-hv'}`}
              >
                {range === 'today' ? 'اليوم' : range === 'week' ? 'أسبوع' : range === 'month' ? 'شهر' : range === 'quarter' ? 'ربع سنة' : range === 'year' ? 'سنة' : 'مخصص'}
              </button>
            ))}
          </div>
